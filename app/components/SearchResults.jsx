...
function ProductCard({url, image, title, price, compare, discount}) {
  ...
  return (
    ...
    >
      <div
        className="rr-product-card rr-search-product-card"
        style={{
          background: '#fff',
      ...
      >
        ...
        <div className="rr-product-card__body" style={{padding: '0.75rem'}}>
          ...
          >
            ...
            {compare &&
              parseFloat(compare.amount) > parseFloat(price?.amount ?? '0') && (
                <Money
                  data={compare}
                  style={{
                    fontSize: '0.75rem',
    ...
    <div style={{marginBottom: '2rem'}}>
      ...
      <div style={{display: 'flex', flexDirection: 'column', gap: '0.5rem'}}>
        {pages.nodes.map((page) => {
          ...
          return (
            <ContentRow key={page.id} url={url} title={page.title} emoji="📋" />
          );
        })}
      </div>
    </div>
...
function ContentRow({url, title, emoji, sub}) {
  ...
  return (
    <Link
      prefetch="intent"
      to={url}
      className="rr-ui-card rr-ui-card--interactive rr-search-content-row"
      style={{
        display: 'flex',
        alignItems: 'center',
        gap: '0.875rem',
