...
export function Rating({value = 0, count, showValue = false, size = 14}) {
  const stars = STAR_POSITIONS.map((position) => {
  ...
  return (
    <div style={{display: 'inline-flex', alignItems: 'center', gap: '0.3rem'}}>
      {showValue && (
        <span
          style={{fontSize: '0.875rem', fontWeight: 800, color: 'var(--ink)'}}
        >
          {value.toFixed(1)}
        </span>
      )}
      <div style={{display: 'flex', gap: '1px'}}>
        {stars.map((type, position) => (
          <Star key={STAR_POSITIONS[position]} type={type} size={size} />
        ))}
      </div>
      {count != null && (
